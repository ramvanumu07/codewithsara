...
export default {
  ...
  "outcome_messages": [
    "**What a class is**\n\nA **class** is a **blueprint** for creating objects with the same shape and behavior. You declare one with **class Name { }** and create an **instance** with **new Name()**. The class describes what properties and methods every instance will have. Think of the class as the type and each **new** call as creating one concrete value of that type. Add a constructor and methods inside the class body to give instances state and behavior.\n\n**Example**\n\n```javascript\nclass Person { }\nconst p = new Person();\nconsole.log(p instanceof Person);\n```\n\n**Output**\n\n```\ntrue\n```\n\n**What happens**\n\n- Person is an empty class; we create one instance with new Person().\n- p is that instance; instanceof Person is true because p was created from Person.\n\n**Practice**\n\nWrite a class Dog with no constructor or methods. Create an instance and assign it to myDog. What is myDog instanceof Dog?",
    "**constructor and this**\n\nThe **constructor** is a special method that runs when you create an instance with **new**. Use it to set up **instance state**: assign properties on **this**, which is the instance being created. Parameters to constructor are what you pass to new MyClass(...). Each instance gets its own copy of the properties you set on this. Don't return a value from constructor—the new object is returned automatically. In **methods**, **this** is the instance the method was called on (e.g. obj.method() → this is obj). Use this to read and write instance properties.\n\n**Example**\n\n```javascript\nclass Point {\n  constructor(x, y) {\n    this.x = x;\n    this.y = y;\n  }\n}\nconst pt = new Point(3, 4);\nconsole.log(pt.x, pt.y);\n```\n\n**Output**\n\n```\n3 4\n```\n\n**What happens**\n\n- new Point(3, 4) runs the constructor with x=3, y=4.\n- this.x and this.y set properties on the new instance; pt gets those values.\n\n**Practice**\n\nAdd a constructor to a class Person that takes name and sets this.name. What does new Person(\"Ali\").name return? In a method, how do you access a property named value set in the constructor?",
    "**Methods**\n\n**Methods** are functions defined on the class; every instance can call them. When you call **instance.method()**, the method runs with **this** set to that instance, so it can read and update instance state. Methods are shared (one function per class) but operate on each instance's data via this. Use methods for actions and computations that belong to the object.\n\n**Example**\n\n```javascript\nclass Greeter {\n  constructor(name) { this.name = name; }\n  greet() { return \"Hi, \" + this.name; }\n}\nconst g = new Greeter(\"Alice\");\nconsole.log(g.greet());\n```\n\n**Output**\n\n```\nHi, Alice\n```\n\n**What happens**\n\n- g is an instance; g.name is \"Alice\" (set in constructor).\n- g.greet() runs with this = g, so this.name is \"Alice\" and the return is \"Hi, Alice\".\n\n**Practice**\n\nAdd a method getArea() to a class Rectangle (with width and height in the constructor) that returns width * height. What does new Rectangle(5, 10).getArea() return?",
    "**static**\n\n**static** marks a member as belonging to the **class itself**, not to instances. Access with **ClassName.staticMethod()** or **ClassName.staticProp**—no instance needed. Use static for **utility functions** (e.g. helpers that don't use instance data), **constants**, or **factory methods**. Inside a static method, this refers to the class. Static members don't have access to instance state unless you pass it in.\n\n**Example**\n\n```javascript\nclass MathUtil {\n  static double(n) { return n * 2; }\n}\nconsole.log(MathUtil.double(5));\n```\n\n**Output**\n\n```\n10\n```\n\n**What happens**\n\n- double is on the class, not on an instance. We call MathUtil.double(5) without new.\n- The method returns 5 * 2.\n\n**Practice**\n\nAdd a static method createDefault() to a class Config that returns new Config() with default values. How do you call it—on an instance or on the class?",
    "**Getters and setters**\n\n**get name() { return this._name; }** and **set name(v) { this._name = v; }** define a property that runs code when **read** or **written**. The user writes obj.name or obj.name = x; the getter or setter runs. Use getters/setters to **validate** (e.g. in the setter), **compute** a value when read, or hide internal storage (e.g. _name inside, expose as name). You can have only a getter (read-only) or only a setter.\n\n**Example**\n\n```javascript\nclass Box {\n  get value() { return this._v; }\n  set value(x) {\n    if (typeof x !== 'number') throw new Error('number only');\n    this._v = x;\n  }\n}\nconst b = new Box();\nb.value = 42;\nconsole.log(b.value);\n```\n\n**Output**\n\n```\n42\n```\n\n**What happens**\n\n- Assigning b.value = 42 runs the setter; it checks the type and sets this._v.\n- Reading b.value runs the getter and returns this._v.\n\n**Practice**\n\nWhy use a getter instead of a plain method getValue()? When would you use a setter that checks the value before assigning?",
    "**Inheritance: extends, super, and overriding**\n\n**class Child extends Parent** makes Child a **subclass** of Parent. Child instances get Parent's methods and can add or **override** their own. In the child's **constructor**, you must call **super(...)** to run the parent constructor (usually before using this). Define a method with the **same name** as the parent to **override** it; the child's version runs on child instances. Use **super.method()** inside the child to call the parent's version. Instances of Child are also **instanceof** Parent.\n\n**Example**\n\n```javascript\nclass Animal {\n  constructor(n) { this.name = n; }\n  speak() { return this.name + \" says hi\"; }\n}\nclass Dog extends Animal {\n  speak() { return this.name + \" barks\"; }\n}\nconst d = new Dog(\"Rex\");\nconsole.log(d.speak());\n```\n\n**Output**\n\n```\nRex barks\n```\n\n**What happens**\n\n- Dog extends Animal; Dog doesn't define its own constructor, so the default one calls super automatically. name is set on the instance.\n- Dog overrides speak(); when we call d.speak(), the Dog version runs and uses this.name.\n\n**Practice**\n\nIn a subclass constructor, when do you have to call super(...)? Parent has greet() returning \"Hello.\" Child overrides it to return \"Hi.\" What does new Child().greet() return?",
    "**instanceof**\n\n**value instanceof Constructor** returns **true** if value is an instance of Constructor (or a subclass). It checks the **prototype chain**: is Constructor.prototype in the object's chain? Use instanceof to **type-check** before calling class-specific methods or to branch logic. For your classes: obj instanceof MyClass after new MyClass(). A subclass instance is also instanceof its parent (e.g. new Dog() is instanceof Animal).\n\n**Example**\n\n```javascript\nclass A { }\nclass B extends A { }\nconst b = new B();\nconsole.log(b instanceof B);\nconsole.log(b instanceof A);\n```\n\n**Output**\n\n```\ntrue\ntrue\n```\n\n**What happens**\n\n- b was created with new B(), so b instanceof B is true.\n- B extends A, so b is also an instance of A; b instanceof A is true.\n\n**Practice**\n\nYou have a variable x that might be an instance of class User or a plain object. Write one condition that is true only when x is a User instance. What does [] instanceof Object evaluate to?"
  ],
  "tasks": [
    {
